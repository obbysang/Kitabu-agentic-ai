...
import { X, Check, Info, AlertTriangle, AlertCircle, Clock, ArrowLeft } from 'lucide-react'
        ...
        <div className="flex flex-col h-full max-h-[500px]">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div className="flex items-center gap-2">
              <button onClick={handleBack} className="p-1 -ml-2 text-slate-500 hover:text-slate-700 rounded-full hover:bg-slate-200 transition-colors">
                <ArrowLeft size={18} />
              </button>
          ...
          <div className="p-6 overflow-y-auto">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex-1">
                <span className="text-xs text-slate-500 font-medium uppercase tracking-wider">{selectedNotification.type}</span>
                <p className="text-xs text-slate-400">{formatTime(selectedNotification.timestamp)}</p>
              </div>
            </div>
            <h4 className="text-lg font-semibold text-slate-900 mb-2">{selectedNotification.title}</h4>
            <p className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap">{selectedNotification.message}</p>
          </div>
          <div className="p-4 border-t border-slate-100 bg-slate-50/50">
            <button 
              onClick={handleBack}
              className="w-full py-2 bg-white border border-slate-200 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors"
            >
              Back to list
            </button>
          </div>
